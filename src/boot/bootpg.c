...
static uintptr_t roundup(uintptr_t n, size_t page_size) {
...
static pte_t *va_set_l2(uint64_t base) {
    ...
    uintptr_t page = (base / LARGE_PAGE_SIZE) & 0x1FF;
...
void page_init(moe_bootinfo_t *_bootinfo, void *mmap, size_t mmsize, size_t mmdescsz) {
    ...
    uintptr_t limit = (last_pa_4G + LARGE_PAGE_SIZE) / LARGE_PAGE_SIZE;
    for (uintptr_t i = 0; i < limit; i++) {
        pte_t la = i * LARGE_PAGE_SIZE;
    ...
    bootinfo->kernel_base = ~max_virtual_address
    ...
    // size_t vram_size = roundup(bootinfo->screen.delta * bootinfo->screen.height * 4, LARGE_PAGE_SIZE) / LARGE_PAGE_SIZE;
    // uintptr_t offset = vram_base / LARGE_PAGE_SIZE;
    // for (uintptr_t i = 0 ; i < vram_size; i++) {
    //     pml2v[offset + i] = (vram_base + i * LARGE_PAGE_SIZE) | common_attributes | PTE_LARGE;
    // }
